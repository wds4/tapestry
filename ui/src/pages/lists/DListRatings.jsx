...
export default function DListRatings() {
  ...
  // Compute parent reference for finding items
  ...
  useEffect(() => {
    ...
    async function fetchItems() {
      ...
      try {
        // Try both item kinds
        const [items9999, items39999] = await Promise.all([
          queryRelay({ kinds: [9999], '#z': [parentRef] }),
          queryRelay({ kinds: [39999], '#z': [parentRef] }),
        ]);
        if (!cancelled) {
          setItems([...items9999, ...items39999]);
        }
      } catch (err) {
  ...
  // Build a map of item event IDs to item names
  const itemMap = useMemo(() => {
    const map = {};
    for (const item of items) {
      const name = getTag(item, 'name') || getTag(item, 'title') || item.id?.slice(0, 12) + '…';
      map[item.id] = name;
    }
    return map;
  }, [items]);
  ...
  // Fetch reactions for ALL items in this list
  const fetchReactions = useCallback(async () => {
    ...
    try {
      ...
      if (pendingSource === SOURCE_LOCAL) {
        // Query local strfry for kind 7 reactions referencing any item
        const batches = await Promise.all(
          itemIds.map(id => queryRelay({ kinds: [7], '#e': [id] }))
        );
        fetchedEvents = batches.flat();
        // Deduplicate
        const seen = new Set();
        ...
        // Query external relays for each item
        const allEvents = [];
        for (const id of itemIds) {
          try {
            ...
            const data = await res.json();
            if (data.success) allEvents.push(...(data.events || []));
          } catch {}
        }
        // Deduplicate
        const seen = new Set();
        fetchedEvents = allEvents.filter(e => {
          if (seen.has(e.id)) return false;
          seen.add(e.id);
          return true;
        });
        ...
        const localAll = localBatches.flat();
        setLocalIds(new Set(localAll.map(e => e.id)));
      }
  ...
  }, [itemIds, pendingSource, relaySets, checkNeo4j]);

  // Auto-fetch when items are loaded
  useEffect(() => {
    if (itemIds.length > 0 && !fetched && !loading) {
      fetchReactions();
    }
  }, [itemIds]);
  ...
  // Include PoV pubkey in profile lookups
  ...
  const rows = useMemo(() => {
    return reactions.map(ev => {
      ...
      let type;
      if (content === '+' || content === '👍' || content === '🤙') type = 'upvote';
      else if (content === '-' || content === '👎') type = 'downvote';
      else type = 'other';

      // Find which item this reaction references
      const eTag = ev.tags?.find(t => t[0] === 'e');
      const targetItemId = eTag?.[1];
      const itemName = targetItemId ? (itemMap[targetItemId] || targetItemId?.slice(0, 12) + '…') : '—';
  ...
  }, [reactions, localIds, neo4jIds, itemMap, trustWeights]);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (val) => {
        ...
        // Color: green for 1, amber for partial, dim for 0
  ...
  return (
    <div>
      ...
      <p className="subtitle">
        Kind 7 (NIP-25) reactions across all items in this list.
      </p>

      {itemsLoading && <p style={{ opacity: 0.6 }}>Loading list items…</p>}

      {!itemsLoading && itemIds.length === 0 && (
        <p style={{ opacity: 0.6 }}>No items found in this list — nothing to show ratings for.</p>
      )}

      {!itemsLoading && itemIds.length > 0 && (
        <>
          <p style={{ fontSize: '0.85rem', opacity: 0.6, marginBottom: '1rem' }}>
            Found {itemIds.length} item{itemIds.length !== 1 ? 's' : ''} in this list.
          </p>
          ...
          {!loading && fetched && (
            <>
              ...
              <DataTable
                columns={columns}
                data={rows}
                emptyMessage="No reactions found for items in this list"
              />
