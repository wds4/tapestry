...
LOCK_FILE="/var/lock/processContentQueue.lock"

# Ensure only one instance runs at a time
exec {LOCK_FD}>${LOCK_FILE}
if ! flock -n ${LOCK_FD}; then
    echo "Another instance of processContentQueue.sh is already running. Exiting."
    exit 1
fi
...
while true; do
    ...
    if [[ "$NUM_FILES" -gt 0 ]]; then
        ...
        # Process one event
        /usr/local/lib/node_modules/brainstorm/src/pipeline/stream/content/updateNeo4jForOneContentEvent.sh
