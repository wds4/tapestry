...
main();// Get all pubkeys from Neo4j using pagination to avoid memory issues
...
async function processEventKind(pubkeysFile, eventKind, relationshipType) {
  try {
    ...
    let strfryOutput = '';
    console.log(`${new Date().toISOString()}: Processing ${pubkeyList.length} pubkeys with strfry for kind ${eventKind}...`);
    
    // Process in smaller batches to avoid overloading the system
    const strfryBatchSize = 100;
    for (let i = 0; i < pubkeyList.length; i += strfryBatchSize) {
      const batchPubkeys = pubkeyList.slice(i, i + strfryBatchSize);
      console.log(`${new Date().toISOString()}: Processing strfry batch ${Math.floor(i/strfryBatchSize) + 1}/${Math.ceil(pubkeyList.length/strfryBatchSize)}`);
      
      for (const pubkey of batchPubkeys) {
        try {
