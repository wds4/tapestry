/**
 * Kind 30382 Export Calculation History API
 * Provides calculation status for customer-specific Kind 30382 Export calculations
 */
...
/**
 * Handler for getting Kind 30382 Export calculation history for a specific customer
 * @param {Object} req - Express request object
 * @param {Object} res - Express response object
 */
async function handleGetHistoryKind30382Export(req, res) {
    try {
        ...
        console.log(`Getting Kind 30382 Export calculation history for customer: ${customerPubkey.substring(0, 8)}...`);
        ...
        const kind30382ExportLogFile = path.join(customerLogDir, 'publishNip85.log');
        
        console.log(`Checking Kind 30382 Export log file: ${kind30382ExportLogFile}`);
        
        // Get calculation status from log file
        const calculationStatus = getKind30382ExportCalculationStatus(kind30382ExportLogFile);
        ...
        res.json({
            ...
            data: {
                ...
                calculation: {
                    type: 'kind30382Export',
                    logFile: kind30382ExportLogFile,
                    ...calculationStatus
                }
            }
        });
        ...
        console.error('Error getting Kind 30382 Export calculation history:', error);
        res.status(500).json({
            success: false,
...
/**
 * Get calculation status from Kind 30382 Export log file
 * @param {string} logFile - Path to the log file
 * @returns {Object} Calculation status information
 */
function getKind30382ExportCalculationStatus(logFile) {
    try {
        ...
        // Check for the most recent "Starting publishNip85" and "Finished publishNip85" entries
        // Expected format: "Tue Jul 29 18:54:47 UTC 2025: Starting publishNip85"
        const startMatches = [...fileContent.matchAll(/(.*?): Starting publishNip85/g)];
        const finishMatches = [...fileContent.matchAll(/(.*?): Finished publishNip85/g)];
