/**
 * Process All Trust Metrics Calculation History API
 * Provides calculation status for customer-specific processCustomer workflow
 */
...
/**
 * Handler for getting Process All Trust Metrics calculation history for a specific customer
 * @param {Object} req - Express request object
 * @param {Object} res - Express response object
 */
async function handleGetHistoryProcessAllTrustMetrics(req, res) {
    try {
        ...
        console.log(`Getting Process All Trust Metrics calculation history for customer: ${customerPubkey.substring(0, 8)}...`);
        ...
        const processCustomerLogFile = path.join(customerLogDir, 'processCustomer.log');
        
        console.log(`Checking Process All Trust Metrics log file: ${processCustomerLogFile}`);
        
        // Get calculation status from log file
        const calculationStatus = getProcessAllTrustMetricsCalculationStatus(processCustomerLogFile);
        ...
        res.json({
            ...
            data: {
                ...
                calculation: {
                    type: 'processAllTrustMetrics',
                    logFile: processCustomerLogFile,
                    ...calculationStatus
                }
            }
        });
        ...
        console.error('Error getting Process All Trust Metrics calculation history:', error);
        res.status(500).json({
            success: false,
...
/**
 * Get calculation status from Process All Trust Metrics log file
 * @param {string} logFile - Path to the log file
 * @returns {Object} Calculation status information
 */
function getProcessAllTrustMetricsCalculationStatus(logFile) {
    try {
        ...
        // Check for the most recent "Starting processCustomer" and "Finished processCustomer" entries
        // Expected format: "Tue Jul 29 18:54:47 UTC 2025: Starting processCustomer"
        const startMatches = [...fileContent.matchAll(/(.*?): Starting processCustomer/g)];
        const finishMatches = [...fileContent.matchAll(/(.*?): Finished processCustomer/g)];
