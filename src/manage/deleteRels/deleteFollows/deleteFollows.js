 ...
 * deleteFollows.js
 * 
 * This script deletes all FOLLOWS relationships from Neo4j in batches
...
function log(message) {
  ...
  // Append to log file
  const logFile = path.join(config.logDir, 'deleteFollows.log');
...
// Get total count of FOLLOWS relationships
function getTotalCount() {
  try {
    log('Counting total FOLLOWS relationships...');
    const query = 'MATCH ()-[r:FOLLOWS]->() RETURN count(r) as count';
    ...
    log(`Found ${count} FOLLOWS relationships to delete`);
    return count;
  } catch (error) {
...
function deleteBatch(batchSize) {
  try {
    const query = `MATCH ()-[r:FOLLOWS]->() WITH r LIMIT ${batchSize} DELETE r RETURN count(r) as deleted`;
    const result = execSync(`sudo cypher-shell -a "${config.neo4jUri}" -u "${config.neo4jUser}" -p "${config.neo4jPassword}" "${query}" --format plain`).toString();
...
async function main() {
  try {
    ensureDirectories();
    log('Starting FOLLOWS relationship deletion process');
    ...
    if (remainingCount === 0) {
      log('SUCCESS: All FOLLOWS relationships have been deleted');
      // Clean up progress file
      if (fs.existsSync(config.progressFile)) {
        fs.unlinkSync(config.progressFile);
      }
    } else if (remainingCount > 0) {
      log(`WARNING: ${remainingCount} FOLLOWS relationships still remain`);
    }
