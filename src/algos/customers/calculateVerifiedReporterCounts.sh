...
# calculate verified reporters for a given customer and store temporarily in NostrUser node
CYPHER1="
MATCH (c1:NostrUserWotMetricsCard {customer_id: $CUSTOMER_ID})<-[:SPECIFIC_INSTANCE]-(:SetOfNostrUserWotMetricsCards)<-[:WOT_METRICS_CARDS]-(f:NostrUser)-[:REPORTS]->(u:NostrUser)
WHERE c1.observee_pubkey = f.pubkey AND c1.influence > 0.01
WITH u, count(f) AS verifiedReporterCount
SET u.customer_verifiedReporterCount=verifiedReporterCount
RETURN count(u) AS numUsersUpdated
"
# optional: RETURN u.pubkey AS nostrUser, verifiedReporterCount ORDER BY verifiedReporterCount DESC

# update NostrUserWotMetricsCard node with verified reporters count
CYPHER2="
...
WITH f, c1, count(f) AS verifiedReporterCount
SET c1.verifiedReporterCount=f.customer_verifiedReporterCount
RETURN count(c1) AS numCardsUpdated
"

# update NostrUser node by setting customer_verifiedReporterCount to NULL
CYPHER3="
MATCH (u:NostrUser)
WHERE u.customer_verifiedReporterCount IS NOT NULL
SET u.customer_verifiedReporterCount = NULL
RETURN count(u) AS numUsersUpdated
"

cypherResults1=$(sudo cypher-shell -a "$NEO4J_URI" -u "$NEO4J_USER" -p "$NEO4J_PASSWORD" "$CYPHER1")

echo "$(date): continuing calculateVerifiedReporterCounts; cypherResults1: $cypherResults1"
echo "$(date): continuing calculateVerifiedReporterCounts; cypherResults1: $cypherResults1" >> ${LOG_FILE}

cypherResults2=$(sudo cypher-shell -a "$NEO4J_URI" -u "$NEO4J_USER" -p "$NEO4J_PASSWORD" "$CYPHER2")

echo "$(date): continuing calculateVerifiedReporterCounts; cypherResults2: $cypherResults2"
echo "$(date): continuing calculateVerifiedReporterCounts; cypherResults2: $cypherResults2" >> ${LOG_FILE}

cypherResults3=$(sudo cypher-shell -a "$NEO4J_URI" -u "$NEO4J_USER" -p "$NEO4J_PASSWORD" "$CYPHER3")

echo "$(date): continuing calculateVerifiedReporterCounts; cypherResults3: $cypherResults3"
echo "$(date): continuing calculateVerifiedReporterCounts; cypherResults3: $cypherResults3" >> ${LOG_FILE}
